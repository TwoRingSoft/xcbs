...
# failure modes
XCBS_EXIT_NO_PATH_PROVIDED=1
XCBS_EXIT_INVALID_XCODEPROJ_PATH=2
XCBS_EXIT_BUILD_SETTINGS_CHANGED=3
...
for XCBS_CONFIGURATION in ${XCBS_CONFIGURATIONS[@]}; do
	for XCBS_SCHEME_PATH in ${XCBS_SCHEMES[@]}; do
		...
		xcodebuild -project "${XCBS_PROJECT}" -scheme "${XCBS_SCHEME_NAME}" -configuration "${XCBS_CONFIGURATION}" -showBuildSettings > "${XCBS_TEMP_OUTPUT_FILE}"
